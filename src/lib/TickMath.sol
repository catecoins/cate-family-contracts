...
/// @notice Uniswap V3 / PancakeSwap V3 TickMath, ported to Solidity 0.8 by
/// wrapping the original (deliberately overflowing) arithmetic in `unchecked`.
/// Behaviour is bit-for-bit identical to the library the live pools use — the
/// fork test suite asserts that by initializing real pools at fuzzed ticks and
/// reading the tick back out of `slot0`.
library TickMath {
    error TickOutOfBounds();
    error SqrtRatioOutOfBounds();
    ...
    function getTickAtSqrtRatio(uint160 sqrtPriceX96) internal pure returns (int24 tick) {
        unchecked {
            ...
            if (!(sqrtPriceX96 >= MIN_SQRT_RATIO && sqrtPriceX96 < MAX_SQRT_RATIO)) revert SqrtRatioOutOfBounds();
